...
resource "aws_spot_instance_request" "this" {
  ...
  provisioner "local-exec" {
    command = "./etc/cloud-init/update_spotinstance_tags.sh ${var.region} ${aws_spot_instance_request.this[each.key].id} ${aws_spot_instance_request.this[each.key].spot_instance_id}"
  }
}
...
resource "aws_ec2_instance_state" "this" {
  ...
  instance_id = aws_spot_instance_request.this[each.key].spot_instance_id
