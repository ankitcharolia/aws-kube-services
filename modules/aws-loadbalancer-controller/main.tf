...
module "aws_loadbalancer_controller_irsa" {
  ...
  role_policy_arns     = aws_iam_policy.aws_loadbalancer_controller_policy.arn
...
module "aws_loadbalancer_controller" {
  ...
  values = yamldecode(templatefile("${path.module}/values.yaml.tftpl", {
    aws_loadbalancer_controller_iam_role_arn = module.aws_loadbalancer_controller_irsa.iam_role_arn
    project                                  = var.project
  }))
  value_files = [
    "$gitRepo/charts/aws-loadbalancer-controller/values.yaml",
  ]
